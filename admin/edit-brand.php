...
?>
    <div class="container">
        ...
        <?php 
            ...
            $row = $objc->getOne(htmlentities(htmlspecialchars(mysqli_real_escape_string($con,$_GET['edit']))),'ep_manufacture');
            $_SESSION['manufacture_id_edit'] = $row['id'];
        ?>
        ...
        <!-- this is for brand -->
                <div class="p-4 border border-dark">
                    ...
                    <div class="mb-3">
                        ...
                        <input value="<?php echo $row['manufacture_name'];?>" name="brandName" type="text" class="form-control" id="exampleFormControlInput1" placeholder="Slider Heading">
                    </div>

                    <input name="id" style="display: none;" value="<?php echo $row['id'];?>">

                    <div class="mb-3 py-2">
                        <input name="ManufactureUpdateBtn" type="submit" class="btn btn-info text-light text-right" value="Save Changes">
                    </div>
                </div>
            </div>
    </div>
